...
open class ApplicationException(
    ...
    fun getErrorMessageString(): String {
        return errorMessageRes?.let { applicationResource.getString(errorMessageRes!!) }
...
sealed class ErrorType {
    sealed class Network(val Code: Int) : ErrorType() {
        ...
        companion object {
            fun getNetworkApplicationException(
                code: Int,
                errorMessage: String?,
                errorCode: String?,
            ): NetworkException {
                return when (code) {
                    BadRequest.Code -> NetworkException(
                        type = BadRequest,
                        errorMessage = errorMessage,
                        errorCode = errorCode
                    )
                    Unauthorized.Code -> NetworkException(
                        type = Unauthorized,
                        errorMessage = errorMessage,
                        errorCode = errorCode
                    )
                    Forbidden.Code -> NetworkException(
                        type = Forbidden,
                        errorMessage = errorMessage,
                        errorCode = errorCode
                    )
                    NotFound.Code -> NetworkException(
                        type = NotFound,
                        errorMessage = errorMessage,
                        errorCode = errorCode
                    )
                    MethodNotAllowed.Code -> NetworkException(
                        type = MethodNotAllowed,
                        errorMessage = errorMessage,
                        errorCode = errorCode
                    )
                    NotAcceptable.Code -> NetworkException(
                        type = NotAcceptable,
                        errorMessage = errorMessage,
                        errorCode = errorCode
                    )
                    PreconditionFailed.Code -> NetworkException(
                        type = PreconditionFailed,
                        errorMessage = errorMessage,
                        errorCode = errorCode
                    )
                    UnsupportedMediaType.Code -> NetworkException(
                        type = UnsupportedMediaType,
                        errorMessage = errorMessage,
                        errorCode = errorCode
                    )
                    InternalServerError.Code -> NetworkException(
